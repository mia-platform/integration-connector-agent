...
|--------------------|----------------------|-----------------------------------------------------|-----------|
| Deployment Hook    | `Deployment Hook`    | [Deployment Event](#deployment-event-payload)       | Write     |
| Feature Flag Hook  | `Feature Flag Hook`  | [Feature Flag Event](#feature-flag-event-payload)   | Write     |
...
| Vulnerability Hook | `Vulnerability Hook` | [Vulnerability Event](#vulnerability-event-payload) | Write     |
...
| Subgroup Hook.     | `Subgroup Hook`      | [Subgroup Event](#subgroup-event-payload)           | Write     |

The GitLab source supports the following resources for full import:

| Resource Type | Event Type                    | Example Payload                                       | Operation |
|---------------|-------------------------------|-------------------------------------------------------|-----------|
| Project       | `gitlab:project:import`       | [Project Import](#project-import-payload).            | Write     |
| Merge Request | `gitlab:merge_request:import` | [Merge Request Import](#merge-request-import-payload) | Write     |
| Pipeline      | `gitlab:pipeline:import`      | [Pipeline Import](#pipeline-import-payload)           | Write     |
| Release       | `gitlab:release:import`       | [Release Import](#release-import-payload)             | Write     |
...
The **event ID** used in the webhook payload is extracted from the `deployment_id` and `project.id` fields and added to
the baseURL set in the source configuration.

<details>
<summary>Deployment Event Payload</summary>
...
The **event ID** used in the webhook payload is extracted from the `object_attributes.id` and `project.id` fields and
added to the baseURL set in the source configuration.

<details>
<summary>Feature Flag Event Payload</summary>
...
The **event ID** used in the webhook payload is extracted from the `object_attributes.id` and `project.id` fields
and added to the baseURL set in the source configuration.
...
<details>
<summary>Pipeline Event Payload</summary>
...
The **event ID** used in the webhook payload is extracted from the `tag` and `project.id` fields and added to
the baseURL set in the source configuration.

<details>
<summary>Release Event Payload</summary>
...
The **event ID** used in the webhook payload is extracted from the `ref` and `project.id` fields and added to
the baseURL set in the source configuration.

<details>
<summary>Tag Push Event Payload</summary>
...
The **event ID** used in the webhook payload is extracted from the `object_attributes.url` and `object_attributes.project_id`
fields and added to the baseURL set in the source configuration.

<details>
<summary>Vulnerability Event Payload</summary>
...
The **event ID** used in the webhook payload is extracted from the `object_attributes.id`, `object_kind`, `event_type`
and `project.id` fields and added to the baseURL set in the source configuration.
...
The **event ID** used in the webhook payload is extracted from the `project_id` field and added to
the baseURL set in the source configuration.
...
#### Subgroup Event Payload

The **event ID** used in the webhook payload is extracted from the `full_path` field and added to
the baseURL set in the source configuration.

<details>
<summary>Subgroup Event Payload</summary>
...
#### Project Import Payload

The **event ID** used in the webhook payload is extracted from the `id` field and added to
the baseURL set in the source configuration.

<details>
<summary>Project Import Payload</summary>
...
#### Merge Request Import Payload

The **event ID** used in the webhook payload is extracted from the `id` field and added to
the project ID correlated to it and the baseURL set in the source configuration.

<details>
<summary>Merge Request Import Payload</summary>
...
#### Pipeline Import Payload

The **event ID** used in the webhook payload is extracted from the `id` field and added to
the project ID correlated to it and the baseURL set in the source configuration.

<details>
<summary>Pipeline Import Payload</summary>
...
#### Release Import Payload

The **event ID** used in the webhook payload is extracted from the `tag_name` field and added to
the project ID correlated to it and the baseURL set in the source configuration.

<details>
<summary>Release Import Payload</summary>
