...
func TestSetupOrganizationSubscriptions(t *testing.T) {
	t.Parallel()
	...
	testCases := map[string]struct {
		...
		"recreate disable hook for one project and skip an enabled one": {
			subscriptionQueryHandler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				defer r.Body.Close()
				w.Header().Set("Content-Type", "application/json")
				_, err := fmt.Fprint(w, `{"results":[{"id":"00000000-0000-0000-0000-000000000001","eventType":"git.repo.created","publisherId":"tfs","consumerId":"webHooks","consumerActionId":"httpRequest","consumerInputs":{"url":"http://example.com/webhook"},"status":"disabled"},{"id":"00000000-0000-0000-0000-000000000002","eventType":"git.repo.deleted","publisherId":"tfs","consumerId":"webHooks","consumerActionId":"httpRequest","consumerInputs":{"url":"http://example.com/webhook"},"status":"enabled"}]}`)
				require.NoError(t, err)
			}),
			expectedCreationCount: 2,
			expectedDeletionCount: 1,
		...
		"skip all hooks": {
			subscriptionQueryHandler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				defer r.Body.Close()
				w.Header().Set("Content-Type", "application/json")
				_, err := fmt.Fprint(w, `{"results":[{"id":"00000000-0000-0000-0000-000000000001","eventType":"git.repo.created","publisherId":"tfs","consumerId":"webHooks","consumerActionId":"httpRequest","consumerInputs":{"url":"http://example.com/webhook"},"status":"enabled"},{"id":"00000000-0000-0000-0000-000000000002","eventType":"git.repo.deleted","publisherId":"tfs","consumerId":"webHooks","consumerActionId":"httpRequest","consumerInputs":{"url":"http://example.com/webhook"},"status":"enabled"}.{"id":"00000000-0000-0000-0000-000000000003","eventType":"git.repo.renamed","publisherId":"tfs","consumerId":"webHooks","consumerActionId":"httpRequest","consumerInputs":{"url":"http://example.com/webhook"},"status":"enabled"}]}`)
				require.NoError(t, err)
			}),
		},
	}
